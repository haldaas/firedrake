...
def test_matis_subdomain_size(target_size, with_bcs):
    """The assembled operator must not depend on how it is decomposed.

    Whatever the subdomains, the MatIS is a partial assembly of the same
    cell-broken matrix, so assembling it fully must reproduce the operator.
    """
...
def test_partition_cells():
    """Subdomains are numbered contiguously, non-empty, and connected."""
...
def test_matis_subdomain_blocks():
    """The subdomain sizes must tile the subdomain matrix exactly."""
...
def test_bddc_subdomain_size_view(tmp_path):
    """PCBDDC must see the subdomains we declared, not fall back to its own."""
...
def test_bddc_subdomain_size_convergence(rg):
    ...
    The subdomain size is fixed in cells, so refining shrinks the subdomains
    along with the mesh and the condition number should settle.  The coarsest
    level is pre-asymptotic, hence the hierarchy starts at 8x8.
    """
...
def test_bddc_subdomain_size_rejected(extra, error):
    """Combinations we do not support must say so, not answer wrongly."""
    mesh = UnitSquareMesh(4, 4)
    V = FunctionSpace(mesh, "CG", 1)
    u, v = TrialFunction(V), TestFunction(V)
    a = inner(grad(u), grad(v))*dx
